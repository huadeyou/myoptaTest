...
/**
 * 参考CuttingStockSample编写
 */
class CuttingStockSample {
...
//                                     0.0, Double.MAX_VALUE));//每种木材所需的切法数除以个数
                ...
                IloColumn col = cutSolver.column(RollsUsed, 1.0).and(cutSolver.column(Fill[j],
                        (int)(_rollWidth/_size[j])));
                cutSolver.numVar(col,0.0, Double.MAX_VALUE);
                cutSolver.exportModel("model2.lp");
//       Cut.add((cutSolver.numVar(col,0.0, Double.MAX_VALUE)));
            ...
            for (;;) {
                //列生成法计算
                cutSolver.exportModel("modelMP2.lp");
                cutSolver.solve();
                //输出共切了多少根17英尺的木材以及每种切法用了多少木材
                ...
                for (int i = 0; i < price.length; i++) {
                    System.out.println("dual_"+i+":"+price[i]);
                }
                ...
                patSolver.exportModel("submodel2.lp");
                patSolver.solve();
                //输出影子价格以及新的切法
                ...
                for (int i = 0; i < newPatt.length; i++) {
                    System.out.println("target_"+i+":"+newPatt[i]);
                }
                //更新数组
