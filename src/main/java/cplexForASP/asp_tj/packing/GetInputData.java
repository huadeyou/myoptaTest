...
public class GetInputData {
    ...
    public void initialData(String filePath,String dateStart) {
        ...
        try {
            ...
            taskMap=new HashMap<>();
            int taskId=0;
            int machineStartCol=3;//产险开始列
            int dueTimeCol=13;//交期时间
            for(int i=startRow;i<totalRows;i++) {
                ...
                if (demandCases!=0) {//仅当需求为0时加入到模型中
                    ...
                    for (int j = machineStartCol; j < machineList.size() + machineStartCol; j++) {
                    ...
                    for (int j = machineList.size() + machineStartCol; j < 2 * machineList.size() + machineStartCol; j++) {
                        humandList.add(Double.valueOf(getExcelDataByIndex(i, j)).intValue());
                    }
                    task.setMachineHumanList(humandList);
                    task.setDueDay(getExcelTimeByIndex(i, dueTimeCol));
                    Integer dueMinute = calTime(startDate, getExcelTimeByIndex(i, dueTimeCol));
