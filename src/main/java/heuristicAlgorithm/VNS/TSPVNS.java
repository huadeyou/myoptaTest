package heuristicAlgorithm.VNS;
...
class Solution //解类
{
    ...
    double getRoute_length()//获取路径长度
    {
        ...
        {
            route_length += TSPVNS.cost_mariax[cur_i][route.get(i)];
            cur_i = route.get(i);
            i++;
        }
        route_length += TSPVNS.cost_mariax[route.get(0)][cur_i];//加回起点
        return route_length;
    }
    void shuffeRoute()//随机生成一个Route
    {
        for(int i = 1; i<= TSPVNS.CUSTOMER_NUMBER; i++)
        {
            this.route.add(i);
        }
    ...
    void print()
    {
        ...
        for(int i = 0; i<= TSPVNS.CUSTOMER_NUMBER-1; i++)
        {
            System.out.print(this.route.get(i));

            if(i== TSPVNS.CUSTOMER_NUMBER-1)
            {
                System.out.print("-");
    ...
    abstract class Strategy//策略类
    {
        ...
        {
            for(int i = 1; i<= TSPVNS.CUSTOMER_NUMBER-1; i++) {
                for (int j = i + 1; j <= TSPVNS.CUSTOMER_NUMBER; j++) {
        ...
        public static Solution shaking(Solution solution, int k)//shaking函数，获得邻域外的解
        {
            ...
            int [] has_exange = new int[TSPVNS.CUSTOMER_NUMBER+1];//储存交换过的解
            for(int i=1;i<=k;i++)//通过k次2-opt获得k重邻域
            {
                Random r = new Random();
                int p = r.nextInt(TSPVNS.CUSTOMER_NUMBER);
                int q = r.nextInt(TSPVNS.CUSTOMER_NUMBER-p)+p+1;
                ArrayList<Integer> new_route = new ArrayList<>();
        ...
        {
            int k = 1;
            while(k<= TSPVNS.CUSTOMER_NUMBER)
            {
                Solution new_solution = new Solution();
                new_solution.route.addAll(solution.route);
                new_solution.route_length = solution.route_length;
